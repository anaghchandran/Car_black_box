...
#define LINE1(x)								      (0x80 + (x))    //for line 1
#define LINE2(x)									  (0xC0 + (x)) //dor line 2.
...
void init_clcd(void);
void clcd_print(const unsigned char *data, unsigned char addr); //to display string
void clcd_putch(const unsigned char data, unsigned char addr); //to dispaly char only
void clcd_write(unsigned char bit_values, unsigned char control_bit); //to display 
